...
export function TransactionItem({
  ...
  return (
    <article className="flex items-center gap-3 rounded-[var(--niva-radius-xl)] px-3 py-3 transition hover:bg-[var(--niva-color-muted-surface)]">
      <div className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-full", positive ? "bg-[var(--niva-color-accent-surface)] text-[var(--niva-color-accent)]" : "bg-[var(--niva-color-muted-surface)] text-[var(--niva-color-danger)]")}>
      ...
      <div className="text-right">
        <p className={cn("text-sm font-bold", positive ? "text-[var(--niva-color-accent)]" : "text-[var(--niva-color-danger)]")}>
          {formatCurrency(movement.amount)}
        </p>
        <p className="mt-0.5 text-xs text-[var(--niva-color-muted)]">{movement.date}</p>
      </div>
      {onEdit || onDelete ? (
        <div className="flex items-center gap-1">
          {onEdit ? <Button type="button" variant="ghost" className="h-9 px-2" onClick={onEdit}>Editar</Button> : null}
          {onDelete ? <Button type="button" variant="ghost" className="h-9 px-2 text-[var(--niva-color-danger)]" onClick={onDelete}>Eliminar</Button> : null}
        </div>
      ) : (
        <MoreHorizontal className="hidden h-4 w-4 text-[var(--niva-color-placeholder)] sm:block" />
      )}
    </article>
  );
}
